...
public class StatisticsResponsitory {
    ...
    public static int getCTVAmount() {
        ...
        try {
            Connection con = DBConnect.getConnection();
            String query = "select count(1) from tblBill t where  t.StatusBill like N'đã%'";
    ...
    try {
        Connection con = DBConnect.getConnection();
        String query = "SELECT COALESCE(SUM(od.PriceAtPuchase), 0) * 0.1 AS ten_percent_total_price\n"
...
//    }
String ctvId = "01797";
        int month = 6; // Example month (July)
        int year = 2024; // Example year

        double monthlyRevenue = getMonthlyRevenueByCTV(ctvId, month, year);
        System.out.println("Monthly revenue for CTV ID " + ctvId + " in " + month + "/" + year + ": " + monthlyRevenue);
           }
